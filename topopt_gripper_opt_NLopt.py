...
class TopOptProblem:
    def __init__(self, nelx, nely, volfrac, penal, rmin, ft, plot_history):
        ...
        # Matches topopt_gripper.py:  dof_out_dist = union1d(load_top, load_bottom)
        ...
        #
        # Reference:
        #   fixed = union1d( dofs[1 : ndof : 2*(nely+1)],
        #                    [2*(nely+1)-2, 2*(nely+1)-1] )
        #
        # dofs[1 :: 2*(nely+1)] picks DOF 1, then every 2*(nely+1)-th DOF after.
        # These are the y-DOF of the bottom node of every column (column 0, 1, …, nelx).
        # With y positive downward and the symmetry plane at y=0 (mesh bottom),
        # zeroing these enforces symmetry: no transverse displacement on the mid-plane.
        #
        # [2*(nely+1)-2, 2*(nely+1)-1] = both DOFs of the top-left corner node
        # (node nely in column 0) — the structural anchor point.
    ...
    def generate_rgb_mesh(self):
        base_gray = (np.round(1.0 - self.xPhys)).flatten()
    ...
    def evaluate(self, x_current):
        ...
        Sign convention is identical to topopt_gripper.py:
          obj  = +u_out  (average output displacement, positive = jaws close)
          dobj = raw adjoint sensitivities (negative for load-path elements)

        NLopt minimises obj with these negative gradients, which drives density
        upward in load-path elements — identical behaviour to the reference MMA.
        No sign flips, no reward terms, no gradient overrides for passive elements.
        """
        ...
        if self.ft == 0:
            ...
            xPhys_f = np.asarray(self.H * x_current[np.newaxis].T / self.Hs)[:, 0]
        ...
        if self.ft == 0:
            ...
            self.cached_dobj[:] = np.asarray(
                self.H * (self.dobj[np.newaxis].T / self.Hs)
            )[:, 0]
            self.cached_dvol[:] = np.asarray(
                self.H * (self.dv[np.newaxis].T / self.Hs)
            )[:, 0] * vol_scalar
...
def main(nelx, nely, volfrac, penal, rmin, ft, plot_history=False, SAVE_FIGURE=True):
    print("Gripper topology optimisation — NLopt MMA")
...
if __name__ == "__main__":
    main(nelx=100, nely=25, volfrac=0.4, rmin=2, penal=3.0,
